...
const Withdrawal = require("../../models/user/withdrawal");
const Ogpa = require("../../models/university/ogpa");
...
exports.updateAffiliate = async (req, res) => {
  ...
  try {
    const affiliate = await Affiliate(estoreid).findByIdAndUpdate(
      affid,
      req.body,
      { new: true }
    );
    ...
    if (!affiliate)
      return res
        .status(404)
        .send(`No affiliate found on Affiliate ID: ${affid}`);
...
exports.updateWithdrawal = async (req, res) => {
  ...
  try {
    if (status === "Approved") {
      ...
      await Affiliate(estoreid).findByIdAndUpdate(
        req.body.affid,
        { status: "Approved" },
        { new: true }
      );
...
exports.updateOgpa = async (req, res) => {
  ...
  try {
    if (status === "active") {
      ogpa = await Ogpa.findByIdAndUpdate(
        ogpaid,
        { ...req.body, md5pass },
        { new: true }
      );
      if (refid)
        await Affiliate(refid).findByIdAndUpdate(
          affid,
          { status: "Approved" },
          { new: true }
        );
