...
exports.populateProduct = async (products, estoreid) => {
  ...
  const categoryList = await Category(estoreid)
    .find({ _id: { $in: categories } })
    .exec();
  const subcatList = await Subcat(estoreid)
    .find({ _id: { $in: subcats } })
    .exec();
  const parentList = await Parent(estoreid)
    .find({ _id: { $in: parents } })
    .exec();
  ...
  products = products.map((product) => {
    return {
      ...
      subcats: product.subcats.map(
        (subcat) =>
          subcatList.filter(
            (sub) => sub._id.toString() === subcat.toString()
          )[0]
      ),
      parent: parentList.filter(
        (par) => par._id.toString() === product.parent.toString()
      )[0],
    };
  });
...
exports.productsProduct = async (source, estoreid) => {
  ...
  const result = await Product(estoreid)
    .find({ _id: { $in: prodId } })
    .exec();
  const products = source[0].products.map((prod) => {
    return {
      ...
      product: result.filter(
        (p) => p._id.toString() === prod.product.toString()
      )[0],
    };
  });
...
exports.orderProductUser = async (source, estoreid) => {
  ...
  const users = await User(estoreid)
    .find({ _id: { $in: popUserId } })
    .exec();

  source = source.map((ord) => {
    return {
      ...(ord._doc ? ord._doc : ord),
      orderedBy: users.filter(
        (u) => u._id.toString() === ord.orderedBy.toString()
      )[0],
    };
  });
