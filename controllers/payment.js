...
exports.listMyPayment = async (req, res) => {
  ...
  const payment = await MyPayment(estoreid).findOne({ _id: req.params.payid }).exec();
  res.json(payment);
};
...
exports.listMyPayments = async (req, res) => {
  ...
  if (address) {
    ...
    payments = await MyPayment(estoreid).aggregate([
        {
          $match: {
            $nor: [
              {
...
exports.createPayment = async (req, res) => {
  ...
  try {
    if (category === "Credit/Debit Card") {
      const existPayment = await MyPayment(estoreid).findOne({ category }).exec();
      if (existPayment) return res.status(400).send(
        "Credit/Debit Card should be one of either Stripe or Paypal only. You already have " + existPayment.name
      );
    }
...
exports.deletePayment = async (req, res) => {
  const estoreid = req.headers.estoreid;
  try {
    const deleted = await MyPayment(estoreid).findOneAndDelete({ _id: req.params.payid });
    if (!deleted) {
