...
exports.list = async (req, res) => {
  ...
  try {
    ...
    const affiliates = await Affiliate(estoreid)
      .find({})
    ...
    const sumCommission = await Affiliate(estoreid)
      .aggregate([
    ...
    const sumWithdraw = await Affiliate(estoreid)
      .aggregate([
    ...
    const totalProducts = await Affiliate(estoreid)
      .find({ commission: { $gte: 0 }, status: "Approved" })
      .exec();
