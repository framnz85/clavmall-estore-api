...
exports.userCart = async (req, res) => {
  ...
  for (let i = 0; i < cart.length; i++) {
    ...
    let productFromDb = await Product(estoreid)
      .findById(cart[i]._id)
      .select("price")
      .exec();
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    products[i].variant = new ObjectId(products[i].variant);
...
exports.saveAddress = async (req, res) => {
  ...
  const { cartTotal } = await Cart(estoreid)
...
exports.addToWishlist = async (req, res) => {
  ...
  if (email) {
    await User(estoreid)
      .findOneAndUpdate({ email }, { $addToSet: { wishlist: productId } })
      .exec();
    list = await User(estoreid).findOne({ email }).select("wishlist").exec();
  } else if (phone) {
    await User(estoreid)
      .findOneAndUpdate({ phone }, { $addToSet: { wishlist: productId } })
      .exec();
...
exports.removeFromWishlist = async (req, res) => {
  ...
  if (email) {
    await User(estoreid)
      .findOneAndUpdate({ email }, { $pull: { wishlist: productId } })
      .exec();
  } else if (phone) {
    await User(estoreid)
      .findOneAndUpdate({ phone }, { $pull: { wishlist: productId } })
      .exec();
  }
...
exports.updateProfile = async (req, res) => {
  ...
  await User(estoreid)
    .findOneAndUpdate({ email: req.user.email }, values)
    .exec();
