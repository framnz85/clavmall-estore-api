...
exports.referralCreate = async (req, res) => {
  ...
  try {
    ...
    if (checkCommission) {
      res.json({
        err: `Referral commission already been given to ${referral.name}`,
      });
    } else {
...
exports.allWithdrawallist = async (req, res) => {
  ...
  try {
    ...
    const withdraws = await Referral(estoreid)
      .find({ username: "Withdraw" })
      .skip((curPage - 1) * pageSize)
      .sort({ [sortkey]: sort })
      .limit(pageSize)
      .exec();
