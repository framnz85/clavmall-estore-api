...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const newCoupon = await Coupon(estoreid).collection.insertOne({ name, code, expiry, discount });
    res.json(newCoupon);
  } catch (error) {
    if (error.code === 11000) {
...
exports.remove = async (req, res) => {
  const estoreid = req.headers.estoreid;
  try {
    res.json(await Coupon(estoreid).findByIdAndDelete(req.params.couponId).exec());
  } catch (error) {
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    res.json(await Coupon(estoreid).findOneAndUpdate(
